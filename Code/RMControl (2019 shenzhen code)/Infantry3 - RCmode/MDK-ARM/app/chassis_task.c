...
//    get_chassis_info();
//    get_chassis_mode();
	temp_chassis = chassis.ctrl_mode;
  ...
  mecanum_calc(chassis.vx, chassis.vy, chassis.vw, chassis.wheel_spd_ref);
	...
	s0 = chassis.wheel_spd_fdb[0];
	s1 = chassis.wheel_spd_fdb[1];
	s2 = chassis.wheel_spd_fdb[2];
	s3 = chassis.wheel_spd_fdb[3];
	
	rs0 = chassis.wheel_spd_ref[0];
	rs1 = chassis.wheel_spd_ref[1];
	rs2 = chassis.wheel_spd_ref[2];
	rs3 = chassis.wheel_spd_ref[3];
...
void follow_gimbal_handler(void)
{
  ...
  if (chassis.follow_gimbal)
			chassis.vw = -pid_calc(&pid_chassis_angle, gim.sensor.yaw_relative_angle, chassis.position_ref);
			//chassis.vw = +rm.vw * CHASSIS_RC_MOVE_RATIO_R + km.vw * CHASSIS_KB_MOVE_RATIO_W;
  else
    chassis.vw = 0;
}
...
void chassis_param_init(void)
{
  ...
  for (int k = 0; k < 4; k++)
  {
    PID_struct_init(&pid_spd[k], POSITION_PID, 10000, 1000, 2.0f, 0, 0);
  }
#endif
