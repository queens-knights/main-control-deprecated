...
//  }
	...
	heat_max = (float)judge_rece_mesg.robot_state.shooter_heat1_cooling_limit;
	heat_gun1 = (float)judge_rece_mesg.power_heat.shooter_heat1;
	gun_speed = (float)judge_rece_mesg.shoot_data.bullet_speed;
	
	
	if ((rc.mouse.l == 1 & (heat_max-heat_gun1)>= 200)||(rc.sw1 == RC_UP & (heat_max-heat_gun1)>= 200))
  {
    trig.one_sta = TRIG_INIT;
    trig.spd_ref = trig.c_shoot_spd;
		debug = 1;
...
void shoot_param_init(void)
{
  ...
  trig.dir             = 0;
  trig.feed_bullet_spd = 6000;
  trig.c_shoot_spd      = -250;
  trig.one_sta         = TRIG_INIT;
